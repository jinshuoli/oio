...
/**
 *说明：号外号封装函数，因interface为微信小程序的关键字，所以修改为oio_interface
 * 时间：2017.06.29
 * 修改：峥嵘时代科技-无敌皮卡秋
 */
function Verification(parm, title, callback) {
    if (parm && parm !== '') {
        extend(parm, title, callback);
    } else {
        callback("传入参数不能为空！", "error");
    }
}

function extend(parm, title, callback) {
    var Default = {
        "rsvd": "0",
        "ver": "1.0",
    };
    ...
    autograph(mergeJson, title, callback);
}
function autograph(mergeJson, title, callback) {
    var sortJson = {};
    Object.keys(mergeJson).sort().forEach(function(item) {
        sortJson[item] = mergeJson[item].toString().toLowerCase();
    });
    var autograph = A(title + JSON.stringify(sortJson).replace(/\:|\,|\"|\'|\{|\}/g, "")).toUpperCase().toString(16); 
    AddSid(autograph, sortJson, callback);
}

function AddSid(autograph, json, callback) {
    json.sid = autograph;
    post(json, callback);
}
...
function post(json, callback) {
    ...
    wx.request({
        ...
        url: "https://weixin1.haowaihao.com.cn/api/vCallReq.do",
...
function oio_interface(json, callback) {
    switch (json.model) {
        case "ax":
            var title = "L9HASRNCM0IQ";delete json.mudel;
            Verification(json, title, callback); break;
        case "axb":
            var title = "L9HASRNCM0IQ";delete json.mudel;
            Verification(json, title, callback); break;
        case "axyb":
            var title = "+ZOLLJ/EFIQEVIWM";delete json.mudel;
            Verification(json, title, callback); break;
        default:
            console.log("请传入正确的模式")
    }
}

function getTs(offset) {
    var date = new Date(),
